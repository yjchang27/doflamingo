...
Ext.define('doflamingo.view.heap_memory.HeapTimeline', {
    ...
    layout          : 'fit',
    chartTitle      : 'Heap Memory Usage',                       // chart title displayed above the chart
    tooltipAppend   : '',                       // text to append to the tooltip value
    alias           : 'widget.HeapTimeline',
    showTooltips    : true,
    showControls    : true,
    ...
    initComponent : function(){
        ...
        Ext.applyIf(this,{
                plain   : true,
                layout  : 'fit',
                html    : '<heapTimeline></heapTimeline>',
                border  : false
            });
    ...
    */
    ...
    drawChart: function(me, width, height) {
        ...
        function setupChart(data) {
            ...
            var selector = "#" + me.getId().toString() + " heapTimeline svg";

            d3.select(selector).remove();
            selector = "#" + me.getId().toString() + " heapTimeline";

            d3.select(selector).append("svg")
                .attr("width", width)
                .attr("height", height - 35);

            selector = "#" + me.getId().toString() + " heapTimeline svg";
            ...
            d3.selectAll("#" + me.getId().toString() + " heapTimeline svg .nv-bar")
                .on('click', function (d) {
                    me.onChartClick(d);
                })

            if(!me.showSeriesLegend){
                d3.select("#" + me.getId().toString() + " heapTimeline svg .nv-legendWrap").remove();
            }
        }
    }
});
