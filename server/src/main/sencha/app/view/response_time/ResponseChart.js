...
Ext.define('doflamingo.view.response_time.ResponseChart', {
    ...
    layout          : 'fit',
    chartTitle      : 'Response Time',                       // chart title displayed above the chart
    tooltipAppend   : '',                       // text to append to the tooltip value
    alias           : 'widget.ResponseChart',
    showTooltips    : true,
    showControls    : true,
    ...
    initComponent : function(){
        ...
        Ext.applyIf(this,{
            plain   : true,
            layout  : 'fit',
            html    : '<responseChart></responseChart>',
            border  : false
        });
    ...
    drawChart: function(me, width, height) {
        ...
        function setupChart(data) {
            ...
            var selector = "#" + me.getId().toString() + " responseChart svg";

            d3.select(selector).remove();
            selector = "#" + me.getId().toString() + " responseChart";

            d3.select(selector).append("svg")
                .attr("width", width)
                .attr("height", height - 35);

            selector = "#" + me.getId().toString() + " responseChart svg";
            ...
            var jsons = testData(['Response Time']);
            var j = 0;
            var playAlert = setInterval(function() {
                ...
                d3.selectAll("#" + me.getId().toString() + " responseChart svg .nv-bar")
                    .on('click', function (d) {
                        me.onChartClick(d);
                    });

                if(!me.showSeriesLegend){
                    d3.select("#" + me.getId().toString() + " responseChart svg .nv-legendWrap").remove();
                }
