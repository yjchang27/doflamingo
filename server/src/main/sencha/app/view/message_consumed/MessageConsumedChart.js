...
Ext.define('doflamingo.view.message_consumed.MessageConsumedChart', {
    ...
    initComponent : function(){
        ...
        Ext.applyIf(this,{
            plain   : true,
            layout  : 'fit',
            html    : '<messageConsumedChart></messageConsumedChart>',
            border  : false
        });
    ...
    drawChart: function(me, width, height) {
        ...
        function setupChart(data) {
            ...
            var selector = "#" + me.getId().toString() + " messageConsumedChart svg";

            d3.select(selector).remove();
            selector = "#" + me.getId().toString() + " messageConsumedChart";

            d3.select(selector).append("svg")
                .attr("width", width)
                .attr("height", height - 35);

            selector = "#" + me.getId().toString() + " messageConsumedChart svg";
            ...
            var playAlert = setInterval(function() {
                ...
                d3.selectAll("#" + me.getId().toString() + " messageConsumedChart svg .nv-bar")
                    .on('click', function (d) {
                        me.onChartClick(d);
                    });

                if(!me.showSeriesLegend){
                    d3.select("#" + me.getId().toString() + " messageConsumedChart svg .nv-legendWrap").remove();
                }
