...
folder are mapped to the application's root namespace, 'doflamingo'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "doflamingo"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'doflamingo' in the same way
as `"doflamingo/sass/src"`.
