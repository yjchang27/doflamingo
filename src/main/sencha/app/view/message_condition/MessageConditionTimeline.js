...
Ext.define('doflamingo.view.message_condtion.MessageCondtionTimeline', {
    ...
    alias           : 'widget.MessageCondtionTimeline',
    showTooltips    : true,
    showControls    : true,
    ...
    initComponent : function(){
        ...
        Ext.applyIf(this,{
                plain   : true,
                layout  : 'fit',
                html    : '<messageCondtionTimeline></messageCondtionTimeline>',
                border  : false
            });
    ...
    */
    ...
    drawChart: function(me, width, height) {
        ...
        if (me.resultData) {
            if (me.localData == '') {
                /*
                 ...
                 is used with extjs does not trip up the json decode that
                 D3.js does.
                 */
        ...
        function setupChart(data) {
            ...
            var selector = "#" + me.getId().toString() + " messageCondtionTimeline svg";

            d3.select(selector).remove();
            selector = "#" + me.getId().toString() + " messageCondtionTimeline";

            d3.select(selector).append("svg")
                .attr("width", width)
                .attr("height", height - 35);

            selector = "#" + me.getId().toString() + " messageCondtionTimeline svg";
            ...
            d3.selectAll("#" + me.getId().toString() + " messageCondtionTimeline svg .nv-bar")
                .on('click', function (d) {
                    me.onChartClick(d);
                })

            if(!me.showSeriesLegend){
                d3.select("#" + me.getId().toString() + " messageCondtionTimeline svg .nv-legendWrap").remove();
            }
        }
    }
});
